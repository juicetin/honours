...
\todo{label mappings - give the `English' labels for the simplified classes}
...
\todo{put some images here from both original and simplified classes}
...
The differences between a Gaussian Process which provides the probability distribution of possible labels compared to the Dirichlet Multinomial Regressor which provides the distribution of actual labels at a point, are highlighted in the illustrative example below. 

\todo{generate toy example with a mixed label A,B region and separate regions of just A, just B}
