...
As the $24$-label case has over $400$ weights, with a little over half converging, the $456$ separate distributions have not been included here, but can be found in the appendix \todo{(not included yet. not sure if worth including in appendix though actually)}. An alternative method to determining the quality of the weights drawn from the $24x19$ dimensions without direclty calculating the r-hat scores would be to use the weights to actually form predictions, and plot their habitat maps - a domain specific approach that most problems would not have the benefit of using. 
...
\begin{figure}[H]
    \centering{\includegraphics[scale=0.77, trim={0 1.5cm 0 0.3cm}]{dm24_argmax_1967.pdf}}
